...
function renderCardArena() {
  ...
  for (let i = 0; i < currentDeckSize; i++) {
    ...
    cardWrapper.className = `card-container ${cardHeightClass} cursor-pointer group mx-auto w-full max-w-[160px]`;

    // Card Inner (handles flip transition)
    const cardInner = document.createElement('div');
    let flippedClass = "";

    if (gameState === 'RESULT' || isEliminated) {
      flippedClass = "is-flipped";
    }

    cardInner.className = `card-inner shadow-xl ${flippedClass}`;
    ...
    if (gameState === 'DECISION') {
      if (isUserPick) {
        ...
        if (currentDeckSize === 3) {
          badgeHTML = `<span class="text-[9px] bg-amber-500 text-slate-950 px-2 py-0.5 rounded-full font-bold uppercase shadow">1st Pick</span>`;
        } else if (currentDeckSize === 10) {
          badgeHTML = `<span class="text-[8px] bg-amber-500 text-slate-950 px-1 py-0.5 rounded font-bold uppercase shadow">1st</span>`;
        } else {
          badgeHTML = `<span class="text-[7px] bg-amber-500 text-slate-950 px-0.5 rounded font-bold">1st</span>`;
        }
    ...
    if (isAce) {
      ...
      // High-contrast playing card face for Kings so revealed cards are immediately distinct from face-down cards
